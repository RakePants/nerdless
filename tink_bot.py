...
model = AutoModelWithLMHead.from_pretrained('./weights/tink_sad')
model = model.to('cpu')
...
async def start(message : types.message):
    ...
    if message.text[1::] == "sad":
        ...
        model = AutoModelWithLMHead.from_pretrained('./weights/tink_vulgar')
    else:
        model = AutoModelWithLMHead.from_pretrained('./weights/tink_toxic')
